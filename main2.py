...
def main():
    ...
    if "syn" in args.setup:
        syn_dataset = MyDatasets.build_image_dataset("saved_images")
        syn_logits = server.zero_shot(syn_dataset, FM, processor, tokenizer, prototype=True)
        accuracy = MyUtils.Evaluate2(
            ground_truth = syn_dataset["label"],
            output_logits = MyUtils.extend_proto_outputs_to_labels({"train":syn_dataset}, syn_logits)
        )
        print(f"Accuracy of the teacher model: {accuracy}%\n")
    ...
    for round_idx in range(args.rounds):
        ...
        for client in clients:
            ...
            elif "md" in args.setup:
                client.local_distillation(agg, prototype=True)
            elif "syn" in args.setup:
                client.local_distillation(syn_logits[client.ID], prototype=True)
    ...
    MyUtils.save_as_json(avg_test_Acc, args, file_name="accuracy_"+args.setup)
